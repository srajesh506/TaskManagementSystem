...
namespace TMS.BusinessLogicLayer
{
    public class TaskManagement
    {
        ...
        public int AddUpdateActivity(Activity activity, Boolean updateFlag = false)
        {
            try
            {
                if (((updateFlag == true) && (GetActivities(activity.ProjectId, false, activity.ActivityId, null).Rows.Count > 0))
        ...
        //if isActive flag is true, it returns active records only
        public DataTable GetSubTasksUsingPaging(out Int32 totalRecords, Int32 pageNum, Int32 pageSize, Int32 projectId, Boolean isActive = false, int subTaskId = -1, int taskId = -1, int activityId = -1, string subTaskName = null)
        {
            try
            {
                SqlCommand sqlCommand = new SqlCommand();
        ...
        public DataTable GetSubTasks(Boolean isActive = false, int subTaskId = -1, int taskId = -1, int activityId = -1, string subTaskName = null, Int32 projectId = -1)
        {
            try
            {
                SqlCommand sqlCommand = new SqlCommand();
        ...
        public int AddUpdateSubtask(SubTask subTask, Boolean updateFlag = false)
        {
            try
            {
                if (((updateFlag == true) && (GetSubTasks(false, subTask.SubTaskId, subTask.TaskId, subTask.ActivityId, null, subTask.ProjectId).Rows.Count > 0))
                    || ((updateFlag == false) && (GetSubTasks(false, subTask.SubTaskId, subTask.TaskId, subTask.ActivityId, subTask.SubTaskName, subTask.ProjectId).Rows.Count <= 0)))
                {
                    SqlCommand sqlCommand = new SqlCommand();
                    sqlCommand.CommandType = CommandType.StoredProcedure;
                    ...
                    sqlCommand.Parameters.Add("@ProjectId", SqlDbType.Int).Value = subTask.ProjectId;
                    sqlCommand.Parameters.Add("@UpdateFlag", SqlDbType.Bit).Value = updateFlag;
                    return dbConnection.ExeNonQuery(sqlCommand);
                }
                else
